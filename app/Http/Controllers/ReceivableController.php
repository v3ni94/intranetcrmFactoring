...
class ReceivableController extends Controller
{
    ...
    {
        abort_unless($receivable->status === 'eingereicht', 422, __('Formale Prüfung nur aus Status "Eingereicht" möglich.'));
        ...
        return back()->with('status', __('Formale Prüfung abgeschlossen: :label', ['label' => $receivable->statusLabel()]));
    }

    public function riskCheck(Request $request, Receivable $receivable, ReceivableRuleEngine $engine)
    {
        abort_unless(in_array($receivable->status, ['formale_pruefung', 'rueckfrage']), 422, __('Risiko-/Limitprüfung in diesem Status nicht möglich.'));
        ...
        if ($result['passed']) {
            ...
            $statusMessage = __('Freigegeben – Regelprüfung ohne Beanstandung.');
        } else {
            $receivable->update([
                'status' => 'rueckfrage',
            ...
            $statusMessage = __('Rückfrage ausgelöst: :reason', ['reason' => $result['reason']]);
        }
    ...
    public function requestSecondVote(Request $request, Receivable $receivable)
    {
        abort_unless(in_array($receivable->status, ['abgelehnt', 'rueckfrage'], true), 422,
            __('Ein Zweitvotum ist nur bei abgelehnten Forderungen oder Rückfragen möglich.'));
        ...
        return back()->with('status', __('Zweitvotum der Marktfolge angefordert.'));
    }
    ...
    public function marketFollowUpVote(Request $request, Receivable $receivable)
    {
        ...
        abort_unless($receivable->status === 'zweitvotum_marktfolge', 422, __('Kein offenes Marktfolge-Votum.'));
        ...
        if ($data['decision'] === 'freigeben') {
            ...
            return back()->with('status', __('Marktfolge hat freigegeben. Ankauf kann berechnet werden (Vier-Augen-Prinzip gilt weiterhin).'));
        }
        ...
        return back()->with('status', __('Marktfolge hat abgelehnt. Der Fall liegt jetzt beim Vorstand.'));
    }
    ...
    public function boardVote(Request $request, Receivable $receivable)
    {
        ...
        abort_unless($receivable->status === 'zweitvotum_vorstand', 422, __('Kein offenes Vorstands-Votum.'));
        ...
        if ($data['decision'] === 'freigeben') {
            ...
            return back()->with('status', __('Vorstand hat freigegeben.'));
        }
        ...
        return back()->with('status', __('Vorstand hat endgültig abgelehnt.'));
    }
    ...
    public function reject(Request $request, Receivable $receivable)
    {
        ...
        abort_unless(
            ...
            422,
            __('Nur Forderungen im Prüfprozess können abgelehnt werden.')
        );
        ...
        return back()->with('status', __('Forderung abgelehnt.'));
    }
}
