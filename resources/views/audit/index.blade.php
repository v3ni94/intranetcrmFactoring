<x-app-layout>
    <x-slot name="header">{{ __('Audit & Freigaben') }}</x-slot>
    ...
    <div class="bg-white rounded-lg border border-aurevia-mist overflow-x-auto mb-6">
        <table class="w-full text-sm">
            <thead class="bg-aurevia-pearl text-[11px] uppercase text-aurevia-label-gray">
                <tr><th class="text-left p-3">{{ __('Zeit') }}</th><th class="text-left p-3">{{ __('Nutzer') }}</th><th class="text-left p-3">{{ __('Aktion') }}</th><th class="text-left p-3">{{ __('Objekt') }}</th><th class="text-left p-3">{{ __('Grund') }}</th></tr>
            </thead>
            <tbody>
            @forelse($events as $e)
                <tr class="border-t border-aurevia-mist/60">
                    <td class="p-3">{{ $e->created_at?->format('d.m.Y H:i') }}</td>
                    <td class="p-3">{{ $e->user->name ?? __('System') }}</td>
            ...
            @empty
                <tr><td colspan="5" class="p-6 text-center text-aurevia-label-gray">{{ __('Keine Audit-Ereignisse vorhanden.') }}</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div class="bg-white rounded-lg border border-aurevia-mist p-4">
        <h2 class="text-sm font-semibold text-aurevia-navy mb-3">{{ __('Letzte Freigabevorgänge (Vier-Augen-Prinzip)') }}</h2>
        @forelse($approvals as $a)
            <div class="text-sm flex justify-between border-b border-aurevia-mist/60 py-1.5">
                ...
                <span>{{ $a->status }} · {{ $a->requester->name ?? '' }} → {{ $a->decider->name ?? __('ausstehend') }}</span>
            </div>
        @empty
            <p class="text-sm text-aurevia-label-gray">{{ __('Keine offenen Freigabevorgänge.') }}</p>
        @endforelse
    </div>
</x-app-layout>
