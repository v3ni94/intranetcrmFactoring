<x-app-layout>
    <x-slot name="header">{{ __('Zahlungseingänge & Abstimmung') }}</x-slot>
    ...
    <div class="mb-4">
        <form method="POST" action="{{ route('payments.import-demo') }}">
            ...
            <button class="text-sm text-white bg-aurevia-navy px-3 py-1.5 rounded-md hover:bg-aurevia-navy/90">
                {{ __('Demo-Kontoauszug importieren (camt.053)') }}
            </button>
        </form>
    </div>
    ...
    <div class="bg-white rounded-lg border border-aurevia-mist overflow-x-auto mb-6">
        <table class="w-full text-sm">
            <thead class="bg-aurevia-pearl text-[11px] uppercase text-aurevia-label-gray">
                <tr>
                    <th class="text-left p-3">{{ __('Wertstellung') }}</th><th class="text-left p-3">{{ __('Verwendungszweck') }}</th>
                    <th class="text-right p-3">{{ __('Betrag') }}</th><th class="text-left p-3">{{ __('Vorschlag') }}</th><th class="p-3"></th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr class="border-t border-aurevia-mist/60">
                    ...
                    <td class="p-3 text-right">
                        ...
                        <form method="POST" action="{{ route('payments.match', $t) }}">
                            ...
                            <button class="text-aurevia-navy hover:underline">{{ __('Bestätigen') }}</button>
                        </form>
                        @endif
                    </td>
                </tr>
            @empty
                <tr><td colspan="5" class="p-6 text-center text-aurevia-label-gray">{{ __('Keine offenen Kontobewegungen.') }}</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div class="bg-white rounded-lg border border-aurevia-mist overflow-x-auto">
        <table class="w-full text-sm">
            <thead class="bg-aurevia-pearl text-[11px] uppercase text-aurevia-label-gray">
                <tr><th class="text-left p-3">{{ __('Wertstellung') }}</th><th class="text-left p-3">{{ __('Zuordnung') }}</th><th class="text-right p-3">{{ __('Betrag') }}</th></tr>
            </thead>
            <tbody>
            ...
            @empty
                <tr><td colspan="3" class="p-6 text-center text-aurevia-label-gray">{{ __('Noch keine zugeordneten Zahlungen.') }}</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
