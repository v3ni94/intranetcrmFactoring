<x-app-layout>
    <x-slot name="header">{{ __('Ankauf & Auszahlungen') }}</x-slot>

    <div class="bg-white rounded-lg border border-aurevia-mist p-6 mb-6">
        <h2 class="text-sm font-semibold text-aurevia-navy mb-3">{{ __('Neuen Auszahlungsbatch bilden') }}</h2>
        @if($readyPurchases->isEmpty())
            <p class="text-sm text-aurevia-label-gray">{{ __('Aktuell keine freigegebenen Ankäufe ohne Auszahlung.') }}</p>
        @else
        <form method="POST" action="{{ route('payouts.store') }}" class="space-y-3">
            @csrf
            <div>
                <x-input-label value="{{ __('Bankkonto') }}" />
    ...
    <div class="bg-white rounded-lg border border-aurevia-mist overflow-x-auto">
        <table class="w-full text-sm">
            <thead class="bg-aurevia-pearl text-[11px] uppercase text-aurevia-label-gray">
                <tr>
                    <th class="text-left p-3">{{ __('Batch') }}</th><th class="text-left p-3">{{ __('Bankkonto') }}</th>
                    <th class="text-right p-3">{{ __('Summe') }}</th><th class="text-left p-3">{{ __('Status') }}</th><th class="p-3"></th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr class="border-t border-aurevia-mist/60">
                    ...
                    <td class="p-3 text-right space-x-2">
                        @if($b->status === 'erstellt')
                            <form class="inline" method="POST" action="{{ route('payouts.approve-first', $b) }}">@csrf<button class="text-aurevia-navy hover:underline">{{ __('1. Freigabe') }}</button></form>
                        @endif
                        @if($b->status === 'freigegeben_1')
                            <form class="inline" method="POST" action="{{ route('payouts.approve-second', $b) }}">@csrf<button class="text-aurevia-navy hover:underline">{{ __('2. Freigabe + SEPA-Export') }}</button></form>
                        @endif
                        @if($b->status === 'angewiesen')
                            <form class="inline" method="POST" action="{{ route('payouts.confirm', $b) }}">@csrf<button class="text-emerald-700 hover:underline">{{ __('Bankbestätigung (Demo)') }}</button></form>
                        @endif
                    </td>
                </tr>
            @empty
                <tr><td colspan="5" class="p-6 text-center text-aurevia-label-gray">{{ __('Noch keine Auszahlungsbatches.') }}</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
