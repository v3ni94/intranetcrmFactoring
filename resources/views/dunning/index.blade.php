<x-app-layout>
    <x-slot name="header">{{ __('Mahnwesen & Streitfälle') }}</x-slot>

    @if($overdueReceivables->isNotEmpty())
    <div class="bg-white rounded-lg border border-aurevia-mist p-6 mb-6">
        <h2 class="text-sm font-semibold text-aurevia-navy mb-3">{{ __('Überfällige Forderungen ohne offenen Fall') }}</h2>
        ...
        <form method="POST" action="{{ route('dunning.store') }}" class="flex items-center gap-3 py-2 border-t border-aurevia-mist/60 text-sm">
            ...
            <select name="case_type" class="text-sm rounded-md border-aurevia-mist">
                <option value="mahnung">{{ __('Mahnung') }}</option>
                <option value="streitfall">{{ __('Streitfall') }}</option>
                <option value="rueckgriff">{{ __('Rückgriff') }}</option>
                <option value="ausfall">{{ __('Ausfall') }}</option>
            </select>
            <button class="text-aurevia-navy hover:underline">{{ __('Fall anlegen') }}</button>
        </form>
        @endforeach
    </div>
    @endif
    ...
    <div class="bg-white rounded-lg border border-aurevia-mist overflow-x-auto">
        <table class="w-full text-sm">
            <thead class="bg-aurevia-pearl text-[11px] uppercase text-aurevia-label-gray">
                <tr>
                    <th class="text-left p-3">{{ __('Forderung') }}</th><th class="text-left p-3">{{ __('Typ') }}</th>
                    <th class="text-right p-3">{{ __('Offener Betrag') }}</th><th class="text-left p-3">{{ __('Status') }}</th><th class="p-3"></th>
                </tr>
            </thead>
            <tbody>
            @forelse($cases as $c)
                <tr class="border-t border-aurevia-mist/60">
                    ...
                    <td class="p-3 text-right space-x-2">
                        ...
                        <form method="POST" action="{{ route('dunning.hand-over', $c) }}" class="inline">
                            @csrf
                            <button class="text-aurevia-navy hover:underline">{{ __('An Inkasso übergeben (Demo)') }}</button>
                        </form>
                        @endif
                        @if($c->status !== 'geschlossen')
                        <form method="POST" action="{{ route('dunning.close', $c) }}" class="inline">
                            @csrf
                            <button class="text-aurevia-navy hover:underline">{{ __('Schließen') }}</button>
                        </form>
                        @endif
                    </td>
                </tr>
            @empty
                <tr><td colspan="5" class="p-6 text-center text-aurevia-label-gray">{{ __('Keine Fälle vorhanden.') }}</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
