<x-guest-layout>
    <h2 class="text-lg font-semibold text-aurevia-navy mb-1">Zwei-Faktor-Authentifizierung</h2>
    <p class="text-sm text-aurevia-label-gray mb-4">
        Für Ihre Rolle verpflichtend (Abschnitt 18). Richten Sie eine Authenticator-App
        (z. B. Google Authenticator, Microsoft Authenticator, Authy) mit dem folgenden
        Schlüssel ein.
    </p>
    ...
    @if(session('recovery_codes'))
        <div class="bg-amber-50 border border-amber-200 rounded-md p-3 mb-4 text-sm">
            <strong>Wiederherstellungscodes (nur jetzt sichtbar, sicher aufbewahren):</strong>
    ...
    @else
        <div class="bg-aurevia-pearl rounded-md p-3 mb-4">
            <p class="text-[11px] uppercase tracking-wide text-aurevia-label-gray mb-1">Manueller Schlüssel</p>
        ...
        <form method="POST" action="{{ route('two-factor.confirm') }}" class="space-y-3">
            @csrf
            <x-input-label for="code" value="6-stelliger Code aus der App" />
    ...
    <form method="POST" action="{{ route('logout') }}" class="mt-6 pt-4 border-t border-aurevia-mist">
        @csrf
        <button type="submit" class="text-sm text-aurevia-label-gray hover:text-aurevia-navy underline">Abmelden</button>
    </form>
</x-guest-layout>
