<x-guest-layout>
    ...
    <p class="text-sm text-aurevia-label-gray mb-4">
        {{ __('Bitte geben Sie den 6-stelligen Code aus Ihrer Authenticator-App ein, oder alternativ einen Ihrer Wiederherstellungscodes.') }}
    </p>

    @if($demoCode)
        <div class="bg-amber-50 border border-amber-200 text-amber-800 rounded-md p-3 mb-4 text-sm">
            {{ __('Demo-Komfort: aktuell gültiger Code für diesen Demo-Zugang:') }} <span class="font-mono font-semibold">{{ $demoCode }}</span>
            {{ __('(30 Sekunden gültig, echte TOTP-Prüfung — kein Bypass).') }}
        </div>
    @endif
