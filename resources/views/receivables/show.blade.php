<x-app-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-lg border border-aurevia-mist p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-sm text-aurevia-label-gray">Kunde: {{ $receivable->organization->name ?? '–' }}</span>
                </div>
                <div class="grid grid-cols-2 gap-2 text-sm mb-4">
                    <div class="text-aurevia-label-gray">Rechnungsnummer</div><div class="text-right">{{ $receivable->invoice_number }}</div>
                    ...
                    <div class="text-aurevia-label-gray">Vertrag</div><div class="text-right">{{ $receivable->contract->contract_number ?? '–' }}</div>
                </div>

                @if($receivable->rejection_reason)
                    <div class="bg-amber-50 border border-amber-200 text-amber-800 text-sm rounded-md p-3 mb-4">
                        <strong>Regel „{{ $receivable->triggered_rule ?? 'manuell' }}“:</strong> {{ $receivable->rejection_reason }}
                    </div>
                @endif
                ...
                <div class="flex flex-wrap gap-2">
                    @if($receivable->status === 'eingereicht')
                        <form method="POST" action="{{ route('receivables.formal-check', $receivable) }}">
                            @csrf
                            <button class="text-sm text-white bg-aurevia-navy px-3 py-1.5 rounded-md hover:bg-aurevia-navy/90">Formale Prüfung durchführen</button>
                        </form>
                    @endif
                    ...
                    @if($receivable->status === 'freigegeben' && !$receivable->purchase)
                        <form method="POST" action="{{ route('purchases.calculate', $receivable) }}">
                            @csrf
                            <button class="text-sm text-white bg-aurevia-gold px-3 py-1.5 rounded-md hover:opacity-90">Ankauf berechnen</button>
                        </form>
                    @endif
                    @if($receivable->status === 'bezahlt')
                        <form method="POST" action="{{ route('payments.settle', $receivable) }}">
                            @csrf
                            <button class="text-sm text-white bg-emerald-700 px-3 py-1.5 rounded-md hover:bg-emerald-800">Abrechnen &amp; Reserve freigeben</button>
                        </form>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg border border-aurevia-mist p-6">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="text-sm font-semibold text-aurevia-navy">Ankaufsberechnung</h3>
                    <span class="text-xs px-2 py-1 rounded bg-aurevia-pearl">{{ ucfirst($receivable->purchase->status) }}</span>
                </div>
                <div class="grid grid-cols-2 gap-2 text-sm mb-3">
                    <div class="text-aurevia-label-gray">Nominalbetrag</div><div class="text-right">{{ eur($receivable->purchase->nominal_amount) }}</div>
                    <div class="text-aurevia-label-gray">Auszahlung (nach Gebühr/Zins)</div><div class="text-right font-semibold text-aurevia-navy">{{ eur($receivable->purchase->immediate_payout_amount) }}</div>
                    ...
                    <div class="text-aurevia-label-gray">Factoringgebühr</div><div class="text-right">{{ eur($receivable->purchase->factoring_fee_amount) }}</div>
                    <div class="text-aurevia-label-gray">Erste Freigabe</div><div class="text-right">Nutzer #{{ $receivable->purchase->approved_by_first }}</div>
                    <div class="text-aurevia-label-gray">Zweite Freigabe</div><div class="text-right">{{ $receivable->purchase->approved_by_second ? 'Nutzer #'.$receivable->purchase->approved_by_second : 'ausstehend' }}</div>
                </div>
                @if($receivable->purchase->needsSecondApproval())
                    <form method="POST" action="{{ route('purchases.approve-second', $receivable->purchase) }}">
                        @csrf
                        <button class="text-sm text-white bg-aurevia-navy px-3 py-1.5 rounded-md hover:bg-aurevia-navy/90">Zweite Freigabe erteilen (Vier-Augen)</button>
                    </form>
                @endif
            </div>
            @endif
        ...
        <div class="bg-white rounded-lg border border-aurevia-mist p-6">
            <h3 class="text-sm font-semibold text-aurevia-navy mb-3">Debitor</h3>
            <p class="text-sm">{{ $receivable->debtorOrganization->name ?? $receivable->debtor_pseudonym_id ?? 'Privat, pseudonymisiert' }}</p>

            <h3 class="text-sm font-semibold text-aurevia-navy mt-6 mb-3">Zahlungen</h3>
            ...
            @empty
                <p class="text-sm text-aurevia-label-gray">Noch keine Zahlung zugeordnet.</p>
            @endforelse
        </div>
    </div>
</x-app-layout>
